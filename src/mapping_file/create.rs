...
use crate::parse_ansi_text::iterators::parse_ansi_split_by_lines_as_spans_iterator::{
    Line, ParseAnsiAsSpansByLinesIterator,
};
...
pub fn create_mapping_file(file_path: PathBuf, contents: String) {
    ...
    let lines_iterators =
        ParseAnsiAsSpansByLinesIterator::create_from_str(contents, ParseOptions::default());
...
pub fn create_mapping_file_from_input_path(
    ...
    let iterator = ParseAnsiAsSpansByLinesIterator::create_from_file_path(
        input_file_path,
...
fn write_mapping_file_from_iterator(file: &mut File, iterator: ParseAnsiAsSpansByLinesIterator) {
    ...
    for line in iterator {
        // append line to file
        file.write(&*create_line_map(line))
...
fn create_line_map(line: Line) -> Vec<u8> {
    let initial_span_for_line = if line.spans.is_empty() {
        ...
        line.spans[0].clone().with_text("".to_string())
    };
    ...
    return [
        initial_style_for_line_ansi_string.as_bytes(),
        first_part_padding.as_bytes(),
        location_in_file.as_slice(),
        DELIMITER.as_bytes(),
    ]
    .concat();
}
