...
mod tests {
    ...
    fn file_output_should_have_line_length_before_first_delimiter() {
        ...
        let lines = mapping_file_content
            .split(|item| *item == b'\n')
            .collect::<Vec<&[u8]>>();

        let first_line =
            String::from_utf8(lines[0].to_vec()).expect("First line should be valid utf8");
    ...
    fn file_input_and_output_should_have_line_length_before_first_delimiter() {
        ...
        create_mapping_file_from_input_path(
            tmp_mapping_file_path.clone(),
            tmp_input_file_path.clone(),
        );
