...
mod tests {
    ...
    fn test_formatter_each_span_is_different_line_and_have_delimiter() {
        ...
        for line in spans_lines {
            for span in line {
                let output_span =
                    sonic_rs::from_str::<Span>(outputs[i].as_str()).expect("Failed to parse span");

                assert_eq!(output_span, span);
                i += 1;
            }

            let output_line_marker = sonic_rs::from_str::<LineMarker>(outputs[i].as_str())
                .expect("Failed to parse line marker");

            assert_eq!(
                output_line_marker,
                LineMarker {
                    type_: "new line".to_string()
                }
            );
